// api/delete-order.js — Elimina pedidos/reseñas/banner, o cancela un pedido devolviendo el stock
...
module.exports = async (req, res) => {
  ...
  try {
    const { orderId, collection, action } = req.body;
    if (!orderId) return res.status(400).json({ error: 'orderId requerido' });
    if (!FIREBASE_SECRET) return res.status(500).json({ error: 'Sin configuración de Firebase' });

    if (action === 'cancel') return await cancelarPedido(orderId, res);
...
async function cancelarPedido(orderId, res) {
  // Leer el pedido
